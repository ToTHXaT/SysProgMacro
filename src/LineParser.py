...
def parse_macrodef(line: str, *, simple=False):
    ...
    args_l = list(i.strip() for i in COMMA_MATCHER.split(_args))
...
def parse_line(line: str) -> Union[Command, MacroDef, Mend]:
    if is_macrodef(line):
    ...
    if length == 0:
        ...
        if cmd in reserved:
            ...
            raise MacroError('-', f'{cmd} is not an operation')
